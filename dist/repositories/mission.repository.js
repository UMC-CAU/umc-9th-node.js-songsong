...
export const addChallenge = async ({ missionId, userId, status }) => {
    ...
    try {
        const [result] = await conn.query("INSERT INTO mission_status (status, mission_id, user_id) VALUES (?, ?, ?)", [status, missionId, userId]);
...
export const getChallenge = async (challengeId) => {
    const conn = await pool.getConnection();
    try {
        const [rows] = await conn.query("SELECT id, mission_id, user_id, status, created_at, updated_at FROM mission_status WHERE id = ?", [challengeId]);
        return rows[0] || null;
    }
    catch (err) {
...
export const checkChallengeStatus = async ({ missionId, userId }) => {
    const conn = await pool.getConnection();
    try {
        const [rows] = await conn.query("SELECT id, status FROM mission_status WHERE mission_id = ? AND user_id = ?", [missionId, userId]);
