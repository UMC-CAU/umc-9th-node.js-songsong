...
export const getMissionStatusId = async ({ userId, missionId }) => {
    ...
    try {
        const [rows] = await conn.query("SELECT id FROM mission_status WHERE mission_id = ? AND user_id = ?", [missionId, userId]);
        // 결과가 없으면 null 반환
        if (rows.length === 0) {
            return null;
        }
...
export const addReview = async ({ content, missionStatusId }) => {
    const conn = await pool.getConnection();
    try {
        const [result] = await conn.query("INSERT INTO review (content, mission_status_id) VALUES (?, ?)", [content, missionStatusId]);
...
export const getReview = async (reviewId) => {
    const conn = await pool.getConnection();
    try {
        const [rows] = await conn.query("SELECT id, content, mission_status_id FROM review WHERE id = ?", [reviewId]);
        if (rows.length === 0) {
